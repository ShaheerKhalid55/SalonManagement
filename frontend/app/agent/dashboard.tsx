...
export default function AgentDashboardScreen(){
 ...
 return <Screen><ScrollView contentContainerStyle={styles.content}
   ...
   <View style={styles.header}>
     <View style={{flex:1}}>
       <AppText style={styles.eyebrow}>SALON AGENT</AppText>
       <AppText style={styles.title}>Hello, {user?.name?.split(" ")[0]||"Agent"}</AppText>
       <AppText style={styles.subtitle}>Today's service desk at a glance.</AppText>
     </View>
     <AppButton title="Sign out" variant="secondary" onPress={logout} style={styles.logout}/>
   </View>

   <View style={styles.hero}>
     <AppText style={styles.heroLabel}>TODAY</AppText>
     <AppText style={styles.heroValue}>{summary?.today_customers_served??0}</AppText>
     <AppText style={styles.heroText}>customers served</AppText>
   ...
   <AppText style={styles.section}>Today's appointments</AppText>
   ...
   <AppText style={styles.section}>Next 7 days</AppText>
   ...
   <AppText style={styles.section}>Last 30 days</AppText>
...
function AppointmentCard({appointment:a,showDate=false}:{appointment:AgentAppointment;showDate?:boolean}){
 return <View style={styles.card}>
   <View style={styles.cardTop}>
     <AppText style={styles.booking}>{a.booking_number}</AppText>
     <View style={styles.status}><AppText style={styles.statusText}>{a.status.replace("_"," ")}</AppText></View>
   </View>
   <AppText style={styles.customer}>{a.customer_name}</AppText>
   {showDate&&<AppText style={styles.meta}>{dateText(a.booking_date)}</AppText>}
   <AppText style={styles.meta}>{time(a.start_time)} - {time(a.end_time)}</AppText>
   <AppText style={styles.services}>{a.service_summary||"Salon service"}</AppText>
   <View style={styles.cardBottom}><AppText style={styles.total}>{money(a.total)}</AppText><AppButton title="Open" variant="secondary" onPress={()=>router.push({pathname:"/agent/booking/[id]",params:{id:String(a.booking_id)}})} style={styles.open}/></View>
 </View>;
}
function Metric({label,value}:{label:string;value:string|number}){return <View style={styles.metric}><AppText style={styles.metricValue}>{value}</AppText><AppText style={styles.metricLabel}>{label}</AppText></View>;}
function Empty({text}:{text:string}){return <View style={styles.empty}><AppText style={styles.emptyText}>{text}</AppText></View>;}
