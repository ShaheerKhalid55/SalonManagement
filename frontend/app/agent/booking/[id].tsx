...
export default function AgentBookingDetails(){
 ...
 if(loading)return <Screen><AppText style={styles.hint}>Loading appointment...</AppText></Screen>;
 if(!booking)return <Screen><AppText style={styles.hint}>Appointment not found.</AppText></Screen>;

 return <Screen><ScrollView contentContainerStyle={styles.content}>
  <AppText style={styles.title}>Customer appointment</AppText>
  <View style={styles.hero}><AppText style={styles.booking}>{booking.booking_number}</AppText><AppText style={styles.customer}>Customer #{booking.customer_id}</AppText><AppText style={styles.date}>{dateText(booking.booking_date)}</AppText><AppText style={styles.time}>{time(booking.start_time)} - {time(booking.end_time)}</AppText><AppText style={styles.status}>{booking.status.replace("_"," ")}</AppText></View>

  <AppText style={styles.section}>Services</AppText>
  <View style={styles.card}>{booking.items.map(i=><View key={i.id} style={styles.item}><AppText style={styles.name}>{i.name}</AppText><AppText style={styles.price}>{money(i.total_price)}</AppText></View>)}</View>
  ...
  {booking.status==="COMPLETED"&&<View style={styles.completed}><AppText style={styles.completedTitle}>Service completed</AppText><AppText style={styles.hint}>This visit has been included in your performance statistics.</AppText></View>}
