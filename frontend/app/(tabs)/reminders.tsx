...
export default function RemindersScreen(){
 ...
 return <Screen><ScrollView contentContainerStyle={styles.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);load();}}/>}>
  <Text style={styles.title}>Reminders</Text><Text style={styles.subtitle}>Don't miss a good time for your next salon visit.</Text>
  {items.length===0?<View style={styles.empty}><Text style={styles.emptyTitle}>No reminders</Text><Text style={styles.text}>When the salon reminder service creates a reminder for you, it will appear here.</Text></View>:
   items.map(r=><View key={r.id} style={styles.card}><Text style={styles.badge}>SALON REMINDER</Text><Text style={styles.cardTitle}>Evening slot available</Text><Text style={styles.text}>{dateText(r.reminder_date)} at {time(r.reminder_time)}</Text><Text style={styles.text}>{r.is_sent?"Notification sent":"Reminder pending"}</Text><AppButton title="View services" onPress={()=>router.push("/(tabs)/services")} style={{marginTop:12}}/></View>)}
 </ScrollView></Screen>;
}
